...
def Fscore(pred: torch.Tensor, y: torch.Tensor,postive_weight:float):
    """Accuracy calculation
    ...
    P = Precision(pred, y)
    R = Recall(pred, y)
    # F = 1 / (2 / (5 * P * (postive_weight+1)) + 3 * postive_weight / (5 * R * (postive_weight+1)) +0.0000001) * (2 + 3 * postive_weight)/ (5 * (postive_weight+1))  #add pos reg
    F = 5 * P * R / (2 * P + 3 * R+0.0000001)
